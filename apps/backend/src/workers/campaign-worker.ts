...
export async function processCampaignJob(
  ...
  updateProgress?: (pct: number) => Promise<void> | void
) {
  const { campaignId, userId, templateId, contactIds } = data
  ...
  for (let i = 0; i < validContacts.length; i++) {
    ...
    if (updateProgress) {
      await updateProgress(Math.round(((i + 1) / validContacts.length) * 100))
    }
  }
...
export function createCampaignWorker() {
  const campaignWorker = new Worker<CampaignJobData>(
    ...
    async (job: Job<CampaignJobData>) => {
      console.log(`[campaign-worker] Starting job ${job.id}`)
      return processCampaignJob(job.data, (pct) => job.updateProgress(pct))
    },
    {
